...
import { Modal, ModalDialog, ModalClose, Box, DialogContent, Button, Typography, FormLabel } from '@mui/joy';
import { useDropzone } from 'react-dropzone';
import Cropper from 'react-easy-crop';

const ChangeProfilePicture = ({ setImage, setLoading, refreshData, session, setOpenSnackbar, setOpenModal, openModal, loading }) => {
    ...
    const onDrop = (acceptedFiles) => {
        ...
        if (!validMimeTypes.includes(file.type)) {
            alert(`Invalid file type: ${file.type}. Only JPEG, PNG, and GIF are allowed.`);
            return;
        }
    ...
    const handleSavePicture = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setOpenSnackbar("Profile picture updated successfully!");
            } else {
                    ...
                    >
                        {previewImage ? (
                            <Box sx={{ position: 'relative', width: '100%', height: 300, display: 'flex', flexDirection: 'column', gap: 2 }}>
                                {/* Cropper Component */}
                                <Box sx={{ position: 'relative', flex: 1, height: '100%', mb: 2 }}>
                                    <Cropper
                                        image={previewImage}
                                        crop={crop}
                                        zoom={zoom}
                                ...
                                <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 2 }}>
                                    <Button onClick={handleSavePicture} disabled={loading} color="primary">
