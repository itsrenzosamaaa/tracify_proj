        ...
        <Grid container spacing={2}>
            <Grid item xs={12} sx={{ height: '90vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Paper elevation={3} sx={{ padding: '2rem', textAlign: 'center' }}>
                    <Typography level="body-lg" fontWeight="700">Welcome back, {session.user.role === 'user' ? session.user.firstname : session.user.role === 'office' ? session.user.roleData.officeName : session.user.username}</Typography>
