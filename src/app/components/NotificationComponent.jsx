...
const NotificationComponent = ({ status, session }) => {
    ...
    const fetchNotifications = useCallback(async () => {
        ...
        try {
            ...
            const sortedNotifications = data.sort((a, b) => new Date(b.dateNotified) - new Date(a.dateNotified));
    ...
    const markAllAsRead = useCallback(async () => {
        try {
            const response = await fetch(`/api/notification/${session.user.id}/read-all`, {
                method: 'PUT',
                headers: {
    ...
    const unreadCount = notifications.filter((notification) => !notification.markAsRead).length;
                                ...
                                <React.Fragment key={notification._id}>
                                    <MenuItem
                                        sx={{
                                            ...
                                            px: 2,
                                            py: 1.5,
                                            backgroundColor: notification.markAsRead ? 'neutral.100' : 'neutral.50',
                                            '&:hover': {
                                                backgroundColor: notification.markAsRead ? 'neutral.200' : 'neutral.100',
                                            }
                                        }}
                                        onClick={() => {
                                        ...
                                        <Box>
                                            ...
                                            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                <Typography level="body-xs" sx={{ color: 'neutral.500', mt: 0.5 }}>
                                                    {formatDistanceToNow(new Date(notification.dateNotified), { addSuffix: true })}
                                                </Typography>
                                                <Typography level="body-xs" sx={{ color: 'neutral.500', mt: 0.5, textDecoration: 'underline' }}>
                                                    Redirect here
                                                </Typography>
                                            </Box>
                                        </Box>
